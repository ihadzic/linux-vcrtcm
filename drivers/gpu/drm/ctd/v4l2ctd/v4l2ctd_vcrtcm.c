		...
		struct v4l2ctd_vcrtcm_hal_descriptor *vhd, int flag)
{
	...
	for (i = 0; i < 2; i++) {
		if (flag == V4L2CTD_ALLOC_PB_FLAG_FB) {
			pbd = &vhd->pbd_fb[i];
...
static int v4l2ctd_alloc_pb(struct v4l2ctd_info *v4l2ctd_info,
	...
	for (i = 0; i < 2; i++) {
		...
		if (r) {
			PR_ERR("%s[%d]: push buffer alloc_failed\n",
					V4L2CTD_ALLOC_PB_STRING(flag), i);
			memset(pbd, 0,
			...
			void *hw_drv_info, int flow)
{
	...
	PR_INFO("Attaching vl2ctd %d to HAL %p\n",
	...
	if (v4l2ctd_info->v4l2ctd_vcrtcm_hal_descriptor) {
		...
		struct v4l2ctd_vcrtcm_hal_descriptor
			*vhd = v4l2ctd_kzalloc(v4l2ctd_info,
		...
		v4l2ctd_info->v4l2ctd_vcrtcm_hal_descriptor = vhd;
			...
			void *hw_drv_info, int flow)
{
	...
	if (vhd->vcrtcm_dev_hal == vcrtcm_dev_hal) {
		...
		v4l2ctd_free_pb(v4l2ctd_info, vhd, V4L2CTD_ALLOC_PB_FLAG_FB);
		v4l2ctd_free_pb(v4l2ctd_info, vhd, V4L2CTD_ALLOC_PB_FLAG_CURSOR);

		v4l2ctd_info->v4l2ctd_vcrtcm_hal_descriptor = NULL;
		v4l2ctd_kfree(v4l2ctd_info, vhd);
...
int v4l2ctd_set_fb(struct vcrtcm_fb *vcrtcm_fb, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("In v4l2ctd_set_fb, minor %d.\n", v4l2ctd_info->minor);
	...
	if (!requested_num_pages) {
		...
		v4l2ctd_free_pb(v4l2ctd_info, vhd,
				V4L2CTD_ALLOC_PB_FLAG_FB);
		...
		r = v4l2ctd_alloc_pb(v4l2ctd_info, vhd,
				requested_num_pages,
				V4L2CTD_ALLOC_PB_FLAG_FB);
		/* shadowbuf init */
...
int v4l2ctd_get_fb(struct vcrtcm_fb *vcrtcm_fb, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("In v4l2ctd_get_fb, minor %d.\n", v4l2ctd_info->minor);
	vhd = v4l2ctd_info->v4l2ctd_vcrtcm_hal_descriptor;
...
int v4l2ctd_xmit_fb(struct drm_crtc *drm_crtc, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("in v4l2ctd_xmit_fb, minor %d\n", v4l2ctd_info->minor);
		...
		void *hw_drv_info, int flow, u32 *status)
{
	...
	if (v4l2ctd_info->status & V4L2CTD_IN_DO_XMIT)
...
int v4l2ctd_set_fps(int fps, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("v4l2ctd_set_fps, fps %d.\n", fps);
	...
	/*schedule_delayed_work(&v4l2ctd_info->fake_vblank_work, 0);*/
	queue_delayed_work(v4l2ctd_info->workqueue, &v4l2ctd_info->fake_vblank_work, 0);
				...
				void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("In v4l2ctd_set_cursor, minor %d\n", v4l2ctd_info->minor);
...
int v4l2ctd_set_dpms(int state, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("in v4l2ctd_set_dpms, minor %d, state %d\n",
...
int v4l2ctd_get_dpms(int *state, void *hw_drv_info, int flow)
{
	...
	PR_DEBUG("in v4l2ctd_get_dpms, minor %d\n",
			v4l2ctd_info->minor);
...
{
	struct v4l2ctd_info *v4l2ctd_info = vhd->v4l2ctd_info;
	unsigned long jiffies_snapshot;
	...
	spin_lock_irqsave(&v4l2ctd_info->v4l2ctd_lock, flags);
	v4l2ctd_info->status |= V4L2CTD_IN_DO_XMIT;
	spin_unlock_irqrestore(&v4l2ctd_info->v4l2ctd_lock, flags);
	...
	if ((vhd->fb_force_xmit ||
		...
		PR_DEBUG("v4l2ctd_do_xmit_fb_push[%d]: frame buffer pitch %d width %d height %d bpp %d\n",
				push_buffer_index,
				vhd->vcrtcm_fb.pitch,
				vhd->vcrtcm_fb.width,
		...
		PR_DEBUG("v4l2ctd_do_xmit_fb_push[%d]: crtc x %d crtc y %d hdisplay %d vdisplay %d\n",
				push_buffer_index,
	...
	if (vhd->pb_needs_xmit[push_buffer_index]) {
		...
		v4l2ctd_info->main_buffer = vhd->pb_fb[push_buffer_index];
		v4l2ctd_info->cursor = vhd->pb_cursor[push_buffer_index];
		...
		PR_DEBUG("v4l2ctd_do_xmit_fb_push[%d]: initiating copy\n",
				push_buffer_index);
		jiffies_snapshot = jiffies;
		mutex_lock(&v4l2ctd_info->sb_lock);
